...
[assembly: PythonModule("zlib", typeof(IronPython2.Zlib.ZlibModule))]

namespace IronPython2.Zlib
{
...
a signed integer.")]
        public static int crc32([BytesConversion]IList<byte> data, long baseValue=0L)
        {
            ...
            if(baseValue >= 0 && baseValue <= uint.MaxValue)
                return IronPython2.Modules.PythonBinaryAscii.crc32(data.ToArray(), (uint)baseValue);
            else
                return IronPython2.Modules.PythonBinaryAscii.crc32(data.ToArray(), (int)baseValue);
        }
