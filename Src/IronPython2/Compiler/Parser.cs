...
using IronPython2.Compiler.Ast;
using IronPython2.Hosting;
        ...
        */
        ...
        private Statement ParseReturnStmt() {
            if (CurrentFunction == null) {
                ReportSyntaxError(IronPython2.Resources.MisplacedReturn);
            }
            NextToken();
        ...
        private Statement ParseYieldStmt() {
            ...
            if (current == null) {
                ReportSyntaxError(IronPython2.Resources.MisplacedYield);
            }
        ...
        private FromImportStatement ParseFromImportStmt() {
            ...
            if (dname.Names.Count == 1 && dname.Names[0] == "__future__") {
                if (!_fromFutureAllowed) {
                    ReportSyntaxError(IronPython2.Resources.MisplacedFuture);
                }
                if (names == FromImportStatement.Star) {
                    ReportSyntaxError(IronPython2.Resources.NoFutureStar);
                }
                fromFuture = true;
                foreach (string name in names) {
                    if (name == "division") {
                        ...
                        if (strName != "braces") {
                            ReportSyntaxError(IronPython2.Resources.UnknownFutureFeature + strName);
                        } else {
                            // match CPython error message
                            ReportSyntaxError(IronPython2.Resources.NotAChance);
                        }
                    }
                }
            }
            ...
            for (int position = 0; ; position++) {
                ...
                if ((parameter = ParseParameter(position, names)) != null) {
                    ...
                    if (MaybeEat(TokenKind.Assign)) {
                        ...
                        ReportSyntaxError(IronPython2.Resources.DefaultRequired);
                    }
                } else {
        ...
        */
        ...
        private Arg FinishKeywordArgument(Expression t) {
            if (!(t is NameExpression n)) {
                ReportSyntaxError(IronPython2.Resources.ExpectedName);
            ...
            if (arg != null && arg.Name != null) {
                for (int i = 0; i < names.Count; i++) {
                    if (names[i].Name == arg.Name) {
                        ReportSyntaxError(IronPython2.Resources.DuplicateKeywordArg);
                    }
                }
            }
        }
        ...
        private Expression FinishTupleOrGenExp() {
            ...
            if (hasRightParenthesis) {
                _sink?.MatchPair(new SourceSpan(_tokenizer.IndexToLocation(lStart), _tokenizer.IndexToLocation(lEnd)),
            ...
            foreach (Arg arg in args) {
                if (arg.Name == null) {
                    if (hasArgsTuple || hasKeywordDict || keywordCount > 0) {
                        ReportSyntaxError(IronPython2.Resources.NonKeywordAfterKeywordArg);
                    }
                } else if (arg.Name == "*") {
                    if (hasArgsTuple || hasKeywordDict) {
                        ReportSyntaxError(IronPython2.Resources.OneListArgOnly);
                    }
                    hasArgsTuple = true; extraArgs++;
                } else if (arg.Name == "**") {
                    if (hasKeywordDict) {
                        ReportSyntaxError(IronPython2.Resources.OneKeywordArgOnly);
                    }
                    hasKeywordDict = true; extraArgs++;
                } else {
                    if (hasKeywordDict) {
                        ReportSyntaxError(IronPython2.Resources.KeywordOutOfSequence);
                    }
                    keywordCount++;
                }
            }
        ...
        private Exception/*!*/ BadSourceError(BadSourceException bse) {
            ...
            return IronPython2.Runtime.Operations.PythonOps.BadSourceError(
                bse._badByte,
