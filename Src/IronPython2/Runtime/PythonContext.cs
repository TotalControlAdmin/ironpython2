...
using PyAst = IronPython2.Compiler.Ast;
...
#region Specialized Hashers
        ...
        public PythonType EnsureModuleException(object key, PythonType baseType, Type underlyingType, PythonDictionary dict, string name, string module, string documentation, Func<string, Exception> exceptionMaker) {
            return (PythonType)(dict[name] = GetOrCreateModuleState(
                key,
        ...
        public override SourceUnit/*!*/ GenerateSourceCode(System.CodeDom.CodeObject codeDom, string path, SourceCodeKind kind) {
            return new IronPython2.Hosting.PythonCodeDomCodeGen().GenerateCode((System.CodeDom.CodeMemberMethod)codeDom, this, path, kind);
        }
#endif
...
#region Stack Traces and Exceptions
        ...
        internal static string FormatPythonSyntaxError(SyntaxErrorException e) {
            var sourceLine = GetSourceLine(e);
            var indent = 0;
            if (sourceLine != null) {
        ...
        private Dictionary<string, Type> CreateBuiltinTable() {
            ...
            // We should register builtins, if any, from IronPython2.dll
            LoadBuiltins(builtinTable, typeof(PythonContext).Assembly, false);

            // Load builtins from IronPython2.Modules
            Assembly ironPythonModules = null;

            try {
                ironPythonModules = DomainManager.Platform.LoadAssembly(GetIronPythonAssembly("IronPython2.Modules"));
            } catch (FileNotFoundException) {
                // IronPython2.Modules is not available, continue without it...
            }
